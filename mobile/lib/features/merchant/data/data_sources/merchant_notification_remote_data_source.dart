...
class MerchantNotificationRemoteDataSource {
  MerchantNotificationRemoteDataSource({
  ...
  Future<List<MerchantNotification>> list({
    ...
    return _extractDataList(
      json,
    ).map(MerchantNotification.fromJson).toList(growable: false);
  }

  Future<MerchantNotification> markAsRead({
    required String accessToken,
    required String id,
  }) async {
    final json = await _send(
    ...
    return MerchantNotification.fromJson(_extractDataMap(json));
  }
